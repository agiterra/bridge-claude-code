  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          machine: { type: "string", description: "Cross-machine spawn: name of a registered crew machine (machine_register) to spawn ON. Non-local machine → REMOTE spawn: crew creates the screen there via ssh + `sudo -u <run_as_uid>`, the agent runs headless (placement ignored), dials the remote's local broker, and its Wire identity is registered against that machine's broker_url (approach A). Omit (or a localhost machine) → unchanged local spawn." },
